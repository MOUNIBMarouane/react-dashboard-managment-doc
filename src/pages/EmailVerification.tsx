  ...
  return <div className="min-h-screen w-full flex">
      ...
      duration: 0.5
        ...
        <div className="w-full max-w-md space-y-8">
          {!verified ? <div className="space-y-8">
              ...
              {error && <Alert className="bg-red-500/10 border-red-500/30 text-red-400">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="flex justify-center text-white \na">
                    <InputOTP maxLength={6} value={verificationCode} onChange={setVerificationCode} render={({
                  slots
                }) => <InputOTPGroup className="gap-3">
                          {[0, 1, 2, 3, 4, 5].map(i => <InputOTPSlot key={i} index={i} className={`w-12 h-14 text-xl font-bold rounded-md border-dashboard-blue-light text-white
                                ${verificationCode.length > i ? "bg-dashboard-accent/20 border-dashboard-accent" : "bg-dashboard-blue-light/50 backdrop-blur-sm border-dashboard-blue-light/70"} 
                                ${verificationCode.length === i ? "ring-2 ring-dashboard-accent/50" : ""}`} />)}
                        </InputOTPGroup>} className="df" />
                  </div>
                ...
                <Button type="button" onClick={handleVerifyCode} className="w-full bg-dashboard-accent hover:bg-dashboard-accent/90 transition-all py-6 text-lg rounded-md shadow-lg shadow-dashboard-accent/20" disabled={isLoading || verificationCode.length !== 6}>
                  {isLoading ? <span className="flex items-center justify-center">
                      ...
                      Verifying...
                    </span> : <span>Verify Email</span>}
                </Button>
                
                <div className="text-center text-sm text-gray-400">
                  <p>
                    Didn't receive a code?{" "}
                    <button type="button" className="text-dashboard-accent hover:text-dashboard-accent-light hover:underline flex items-center justify-center mx-auto mt-2" onClick={handleResendCode} disabled={isResending}>
                      {isResending ? <>
