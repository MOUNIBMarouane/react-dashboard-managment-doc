...
const Layout = ({
  ...
  return <div className="h-screen w-full flex bg-dashboard-blue overflow-hidden">
      {/* Sidebar - Fixed */}
      {isMobile ? <div className={`fixed inset-0 z-30 ${showMobileSidebar ? 'block' : 'hidden'}`}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="h-16 border-b border-white/10 px-4 md:px-6 flex items-center justify-between sticky top-0 z-10 bg-dashboard-blue bg-opacity-80 backdrop-blur-sm">
          <div className="flex items-center gap-4">
            {isMobile && <button onClick={toggleMobileSidebar} className="p-2 rounded-lg hover:bg-white/10 text-white">
                <Menu size={20} />
              </button>}
