...
const SubTypeManagementPage = () => {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div
        className={`${
          isDark ? "bg-[#0a1033] border border-blue-900/30" : "page-header"
        } rounded-lg p-6 mb-6 transition-all`}
      >
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1
              className={`text-2xl md:text-3xl font-semibold mb-2 ${
                isDark ? "text-white" : "page-header-title"
              } flex items-center`}
            >
              <Layers
                className={`mr-3 h-6 w-6 ${
                  isDark ? "text-blue-400" : "page-header-icon"
                }`}
              />{" "}
              Subtype Management
            </h1>
            <p
              className={`text-sm md:text-base ${
                isDark ? "text-gray-400" : "page-header-description"
              }`}
            >
      ...
      {/* Filter bar */}
      <div className={isDark ? "" : "filter-bar"}>
        <SubTypeFilterBar
