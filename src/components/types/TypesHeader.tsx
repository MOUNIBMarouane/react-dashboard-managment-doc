...
import { Trash, Tag, Search, Plus, Filter } from "lucide-react";
import { Input } from "@/components/ui/input";

interface TypesHeaderProps {
  selectedTypes: string[];
  onOpenDeleteDialog: () => void;
  onOpenAddTypeDialog: () => void;
  searchQuery: string;
  onSearchChange: (query: string) => void;
}

const TypesHeader: React.FC<TypesHeaderProps> = ({
  selectedTypes,
  onOpenDeleteDialog,
  onOpenAddTypeDialog,
  searchQuery,
    ...
    <div className="flex flex-col gap-4 mb-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
        <div className="flex items-center mb-4 md:mb-0">
          <Tag className="mr-2 text-dashboard-accent" />
          <h1 className="text-2xl font-bold text-white">Types</h1>
        </div>
        
        <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
          <Button
            onClick={onOpenAddTypeDialog}
      ...
      <div className="relative">
        ...
        <Input
          type="text"
          placeholder="Search types by name or attribute..."
          value={searchQuery}
